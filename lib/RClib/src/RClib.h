...
//Change the following values according to the measured values of your RC module:
...
namespace RClib
{
    template <typename T>
        ...
        {
            static int32_t temp = (num - rem.xMin) * (outMax - outMin) / (rem.xMax - rem.xMin) + outMin;
            return (temp);
        }
...
class RC
{
   public:
    ...
    explicit RC(const uint8_t &numChannels, const uint8_t *channelPins);
